...
def select_map(blended: list[float],
               ...
               *,
               ...
               late_start: int = 31,
               audio_veto: int = 6) -> list[int]:
    blocks = len(blended)
    removed_count = blocks - keep
    if removed_count <= 0:
        return list(range(blocks))
    if min_late_removals > removed_count:
        raise ValueError("min_late_removals exceeds the removal budget")
    veto = set(int(i) for i in sorted(range(blocks), key=lambda i: -audio[i])[:audio_veto])
    protected = veto | set(range(keep_prefix)) | {blocks - 1}

    def contiguous_ok(removed: set[int]) -> bool:
        run = 0
        for index in range(blocks):
            run = run + 1 if index in removed else 0
            if run > max_contiguous_removed:
                return False
        return True

    late_pool = [i for i in range(late_start, blocks - 1) if i not in protected]
    early_pool = [i for i in range(keep_prefix, late_start) if i not in protected]
    removed: set[int] = set()
    ...
    return [i for i in range(blocks) if i not in removed]
