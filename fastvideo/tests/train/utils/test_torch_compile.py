...
def test_regional_compile_unsupported_for_vsa_backends() -> None:
    ...
    all-to-alls plus a host-synced metadata guard) are not fullgraph-traceable.
    enable_torch_compile is a per-run switch shared by every DMD role, so the
    loader must skip the VSA student while dense FLASH_ATTN/SDPA roles compile.
    """
    from fastvideo.models.loader.fsdp_load import _regional_compile_unsupported_reason
    ...
    for backend in (AttentionBackendEnum.VIDEO_SPARSE_ATTN, AttentionBackendEnum.VIDEO_SPARSE_ATTN_H3):
        config = _Config()
        config._resolved_attention_backend = backend
        reason = _regional_compile_unsupported_reason({"config": config})
        assert reason is not None
        assert backend.name in reason

    sdpa_config = _Config()
    sdpa_config._resolved_attention_backend = AttentionBackendEnum.TORCH_SDPA
    assert _regional_compile_unsupported_reason({"config": sdpa_config}) is None
    assert _regional_compile_unsupported_reason({"config": None}) is None
