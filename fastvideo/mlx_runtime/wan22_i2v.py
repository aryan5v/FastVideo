    ...
    if not isinstance(noise_latents, np.ndarray):
        ...
        if isinstance(noise_latents, mx.array):
            ...
            if not isinstance(img, mx.array):
                # Torch tensors on MPS cannot be handed to NumPy directly. The
                # I2V preparation path is intentionally CPU/host-mediated until
                # the VAE parity gate is complete, so make that transfer
                # explicit and fail nowhere later inside mx.array.
            ...
            if tuple(img[:, :, 0].shape) != tuple(noise_latents[:, :, 0].shape):
                raise ValueError(
                    f"image frame shape {tuple(img[:, :, 0].shape)} != "
                    f"latent frame 0 shape {tuple(noise_latents[:, :, 0].shape)}")
