    ...
    *,
    ...
    for block_index in range(total_frames // nfb):
        ...
        model.forward_chunk(current,
                            ...
                            crossattn_caches,
                            current_start=current_start)
        # The context-update output is discarded, so force its cache writes now.
        # Otherwise MLX retains an O(T) lazy graph even though cache tensors are
        # bounded, invalidating the long-rollout memory guarantee.
        mx.eval(current, *[cache.k for cache in kv_caches], *[cache.v for cache in kv_caches])
