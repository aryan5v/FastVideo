...
class MLXCausalWanDiT:
    ...
    def _condition(self, timestep, encoder_hidden_states) -> tuple:
        ...
        if encoder_hidden_states.shape[1] > self.text_len:
            # Match the reference model's fixed text context rather than
            # letting a long prompt overflow the cached cross-attention shape.
    ...
    def forward_chunk(
        ...
        *,
        ...
        if len(kv_caches) != len(self.blocks) or len(crossattn_caches) != len(self.blocks):
            raise ValueError("KV and cross-attention cache counts must match the transformer block count")
        for block, kv_cache, crossattn_cache in zip(self.blocks, kv_caches, crossattn_caches, strict=True):
            hidden = block(hidden,
                           ehs,
                           timestep_proj,
                           cos,
