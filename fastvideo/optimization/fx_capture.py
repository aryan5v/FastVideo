...
# Public dispatch-facing capture API. Keeping these operations here makes
# exported/runtime fingerprints share one implementation without forcing the
# dispatcher to import underscore-private capture internals.
def invocation_input_signatures(
    args: tuple[Any, ...],
    kwargs: dict[str, Any],
) -> list[dict[str, Any]]:
    """Return metadata-only tensor signatures for a module invocation."""
    return _input_metas(args, kwargs)


def invocation_output_signatures(output: Any) -> list[dict[str, Any]]:
    """Return metadata-only tensor signatures for a module output."""
    return _output_metas(output)


def invocation_shape_key(metas: list[dict[str, Any]]) -> str:
    """Return the canonical capture shape key for an invocation."""
    return _shape_key(metas)


def capture_invocation_identity(
    module: nn.Module,
    args: tuple[Any, ...],
    kwargs: dict[str, Any],
    output: Any,
    *,
    ...
    """Capture one observed invocation and return its region identity.

    Failures are raised because dispatch needs one definitive decision rather
    than a full discovery report containing graph-break records.
    """
